...
@Component({
  ...
  <div class="container-fluid" *ngIf="model">
    <h3 class="mb-3">{{model._id ? 'Edit' : 'Create'}} Tag</h3>
...
export class TagFormComponent implements OnInit {
  model: BlogTag | any = { name: '', slug: '' };
  id: string | null = null;
  saving = false; error = '';
  constructor(private route: ActivatedRoute, private router: Router, private svc: TagsService) {}
  ...
  save() {
    ...
    obs.subscribe({ next: ()=> this.router.navigate(['/blog/tags']), error: (e)=> { this.error = e?.error?.message || 'Save failed'; this.saving = false; } });
  }
  back() { this.router.navigate(['/blog/tags']); }
}
